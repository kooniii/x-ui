...
while true; do
    ...
    if $install_socks5; then
        socks5_inbound=$(generate_socks5_inbound)
        new_inbounds+=("$socks5_inbound")
    fi
    if $install_vmess; then
        random_uuid=$(generate_random_uuid)
        vmess_inbound=$(generate_vmess_inbound "$random_uuid")
        new_inbounds+=("$vmess_inbound")
    fi
    if $install_shadowsocks; then
        shadowsocks_inbound=$(generate_shadowsocks_inbound)
...
EOF
    ...
    if $install_socks5; then
        socks5_output=""
        if [[ $ipv4_address =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            socks5_output+="${ipv4_address}:${FIXED_PORT}:${FIXED_USER}:${FIXED_PASS}\n"
        fi
        if [[ $ipv6_address =~ ^[0-9a-f:]+$ ]]; then
            socks5_output+="[${ipv6_address}]:${FIXED_PORT}:${FIXED_USER}:${FIXED_PASS}\n"
        fi
        echo -e "\n--- Socks5 连接信息 ---"
    ...
    if $install_shadowsocks; then
        ...
        if [[ $ipv4_address =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            ss_base_str="chacha20-ietf-poly1305:${FIXED_PASS}@${ipv4_address}:${FIXED_PORT}"
            ss_base64=$(echo -n "$ss_base_str" | base64 | tr -d '\n')
            shadowsocks_output+="ss://${ss_base64}#${ps_ipv4}\n"
        fi
        if [[ $ipv6_address =~ ^[0-9a-f:]+$ ]]; then
            ss_base_str="chacha20-ietf-poly1305:${FIXED_PASS}@[${ipv6_address}]:${FIXED_PORT}"
            ss_base64=$(echo -n "$ss_base_str" | base64 | tr -d '\n')
